...
public abstract class JogoSudoku implements JogoInterface {
    ...
    protected String verificarJogo() 
    {
    ...
    protected void adicionarJogada(Scanner scanner)
    {
        int maximoTentativas = Constants.MAX_TENTATIVAS;
        try 
        {
            for (int tentativa = 0; tentativa < maximoTentativas; tentativa++) 
            {
                ...
                {
                    System.out.println("Formato inválido. Precisa ser (linha, coluna, valor)");
                    continue;
                }
                ...
                for (String jogadaIndividual : jogadas) 
                {
                    ...
                    {
                        System.out.println("Erro! Valores fora do intervalo.");
                        continue;
                    }

                    if (posicaoEstaPreenchida(linha - 1, coluna - 1)) 
                    { 
                        System.out.printf("Erro! A posição (%d,%d,%d) já está preenchida.%n", linha, coluna, valor);
                        continue;
                    }
